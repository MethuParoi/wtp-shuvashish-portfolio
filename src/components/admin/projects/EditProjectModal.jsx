...
export default function EditProjectModal({ project, onSave, onClose }) {
  ...
  try {
    ...
    if (imageFile) {
      ...
      const bucketId = uploadedFile.bucketId;
      const fileId = uploadedFile.$id;
      const endpoint = process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT; // Fixed variable name
      imageUrl = `${endpoint}/storage/buckets/${bucketId}/files/${fileId}/view?project=${process.env.NEXT_PUBLIC_APPWRITE_PROJECT}&mode=admin`;
    }
    ...
    // Remove the commented conditional logic - we want to always update slug when title changes
    // The slug should be updated whenever the title is updated
